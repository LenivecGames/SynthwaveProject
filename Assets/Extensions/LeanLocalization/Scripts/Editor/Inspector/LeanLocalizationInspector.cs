...
namespace Lean.Localization
{
	...
	public class LeanLocalizationInspector : Editor
	{
		...
		static LeanLocalizationInspector()
		{
		...
		public override void OnInspectorGUI()
		{
            ...
            {
                localization.hideFlags = HideFlags.NotEditable;
            }
        }
		...
		{
			Rect labelA = Reserve();
			Rect valueA = Reserve(ref labelA, 35.0f);
			...
			if (GUI.Button(valueA, "Add") == true)
			{
				...
				LeanPhrase newPhrase = localization.AddPhrase("New Phrase");
			...
			for (var i = 0; i < localization.Phrases.Count; i++)
			{
				LeanPhrase phrase = localization.Phrases[i];
				Rect labelB = Reserve();
				Rect valueB = Reserve(ref labelB, 20.0f);
				...
				if (phraseIndex == i)
				{
					...
					{
						MarkAsModified();
                        if (EditorUtility.DisplayDialog("Remove Phrase", "Remove phrase: " + localization.Phrases[i].Name + "/n You cannot undo this action.", "Remove", "Cancel"))
                        {
